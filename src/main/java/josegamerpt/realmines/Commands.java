...
public class Commands implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if ((sender instanceof Player)) {
			...
			MinePlayer mp = PlayerManager.searchPlayer(p);
			if ((cmd.getName().equalsIgnoreCase("mine")) && (p.hasPermission("RealMines.Admin"))) {
				if (args.length == 0) {
					printHelp(p);
				} else if (args.length == 1) {
					if (args[0].equals("create")) {
						Text.send(p, RealMines.getPrefix() + "&cWrong usage. &f/mine create <name>");
					} else if (args[0].equals("wand")) {
						Text.send(p, RealMines.getPrefix()
								+ "&fYou can use the &aSelection Tool &fto select a region for the new mine.");
						...
						Text.send(p, RealMines.getPrefix() + "&aReloaded.");
					} else {
						Text.send(p, RealMines.getPrefix() + "&fNo command has been found with that syntax.");
					}
					...
					if (args[0].equals("create")) {
						...
						if (MineManager.exists(name) == false) {
							...
							Text.send(p, RealMines.getPrefix() + "&cThere is already a mine with that name.");
						}
					} else if (args[0].equals("settp")) {
						if (MineManager.exists(args[1]) == true) {
							Mine m = MineManager.getMine(args[1]);
							m.teleport = p.getLocation();
							m.saveData(Data.TELEPORT);
							mp.sendMessage("&fTeleport &Aset.");
						} else {
							Text.send(p, RealMines.getPrefix() + "&cNo mine exists with that name.");
						}
						...
						if (MineManager.exists(args[1]) == true) {
							...
							Text.send(p, RealMines.getPrefix() + "&fMine deleted.");
						} else {
							Text.send(p, RealMines.getPrefix() + "&cNo mine exists with that name.");
						}
						...
						if (MineManager.exists(args[1]) == true) {
							Mine m = MineManager.getMine(args[1]);
							m.clear();
							mp.sendMessage(Text.addColor("&fMine has been &acleared."));
						} else {
							Text.send(p, RealMines.getPrefix() + "&cNo mine exists with that name.");
						}
						...
						if (MineManager.exists(name) == true) {
							...
							Text.send(p, RealMines.getPrefix() + "&cThere is no mine with that name.");
						}
					} else {
						Text.send(p, RealMines.getPrefix() + "&fNo command has been found with that syntax.");
					}
				...
				Text.send(p, RealMines.getPrefix() + nop);
			}
		} else {
	...
	private void printHelp(Player p) {
		Text.sendList(p,
				Arrays.asList("", "        &9Real&bMines", "&7Release &a" + RealMines.pl.getDescription().getVersion(), "",
						"/mine create <name>", "/mine list", "/mine wand", "/mine settp", "/mine setregion",
